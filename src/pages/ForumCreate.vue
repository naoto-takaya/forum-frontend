        ...
        <v-form @submit.prevent="submitForum" ref="form">
          ...
          <div class="image">
            <v-file-input
              ...
              label="画像を添付"
              accept="image/*"
              @change="postRekognition"
            ></v-file-input>
          </div>
          <p class="image-description">
            添付される画像はAIによって解析されます。
            <br />投稿に相応しくないと判断された場合、画像は投稿できません。
          </p>
          <div class="image-upload">
            <v-img :src="preview" aspect-ratio="1" class="preview" v-if="preview">
...
export default {
  ...
  methods: {
    ...
    submitForum: async function() {
      ...
      await this.axios.post("/forums", formData, config).then(response => {
    ...
    postRekognition: async function(file) {
      ...
      switch (true) {
        case this.level == null:
          this.levelResult =
            "画像解析に失敗しました。時間をおいてお試しいただくか別の画像を使用してください";
          break;
        case this.level === 0:
          this.levelResult =
            "この画像はセンシティブな内容を含んでいるため、投稿できません";
          break;
        default:
